...
class MemoryExtractor:
    ...
    def __init__(self):
        """初始化记忆提取器"""
        self.client = OpenAI(api_key=Config.OPENAI_API_KEY, base_url=Config.API_BASE_URL)
    ...
    def extract_memories(
        ...
        # 2. 使用LLM提取更复杂的记忆点
        llm_based_memories = self._extract_by_llm(
    ...
    def _extract_by_rules(
        self, message: str, emotion: Optional[str] = None, intensity: Optional[float] = None
    ) -> List[Dict[str, Any]]:
        """基于规则提取记忆"""
        memories = []
        ...
        # 规则4: 检测承诺和计划
        commitment_patterns = [r"我会|我要|我打算|我决定|我承诺", r"不再|以后|从今天开始"]
...
如果没有值得记忆的信息，返回空数组。"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "你是一个专业的记忆提取助手，善于识别对话中的关键信息。",
                    },
    ...
    def _calculate_importance(
        self, message: str, emotion: Optional[str], intensity: Optional[float]
    ) -> float:
        """计算记忆重要性（0-1）"""
    ...
    def _deduplicate_and_rank(self, memories: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for mem in memories:
            mem_type = mem["type"]
            if (
                mem_type not in unique_memories
                or mem["importance"] > unique_memories[mem_type]["importance"]
            ):
                unique_memories[mem_type] = mem

        # 按重要性排序
        sorted_memories = sorted(
            unique_memories.values(), key=lambda x: x["importance"], reverse=True
        )

        return sorted_memories

    def should_extract_memory(
        self, user_message: str, emotion: Optional[str] = None, intensity: Optional[float] = None
    ) -> bool:
        """
        判断是否应该从这次对话中提取记忆
