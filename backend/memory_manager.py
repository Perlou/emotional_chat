...
class MemoryManager:
    ...
    def process_conversation(
        ...
        for memory in memories:
            stored_memory = self.store_memory(user_id=user_id, session_id=session_id, memory=memory)
            if stored_memory:
    ...
    def store_memory(
        self, user_id: str, session_id: str, memory: Dict[str, Any]
        ...
        try:
            ...
            self.memory_collection.add(
                documents=[memory_text], metadatas=[metadata], ids=[memory_id]
            )
    ...
    def get_user_emotion_trend(self, user_id: str, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            # 查询所有该用户的记忆（通过空查询 + 过滤）
            results = self.memory_collection.get(where={"user_id": user_id}, limit=100)
            ...
            if results and results.get("metadatas"):
                for metadata in results["metadatas"]:
                    ...
                    recent_emotions.append(
                        {"emotion": emotion, "intensity": intensity, "timestamp": timestamp_str}
                    )
            ...
            if recent_emotions:
                ...
                if len(recent_emotions) >= 3:
                    early_avg = sum(
                        e["intensity"] for e in recent_emotions[: len(recent_emotions) // 2]
                    ) / (len(recent_emotions) // 2)
                    late_avg = sum(
                        e["intensity"] for e in recent_emotions[len(recent_emotions) // 2 :]
                    ) / (len(recent_emotions) - len(recent_emotions) // 2)
