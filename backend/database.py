...
class UserFeedback(Base):
    ...
    feedback_type = Column(
        String(50)
    )  # irrelevant(答非所问), lack_empathy(缺乏共情), overstepping(越界建议), helpful(有帮助), other
...
class DatabaseManager:
    ...
    def save_message(
        self, session_id, user_id, role, content, emotion=None, emotion_intensity=None
    ):
        """保存聊天消息"""
        message = ChatMessage(
    ...
    def save_emotion_analysis(
        self, session_id, user_id, message_id, emotion, intensity, keywords, suggestions
    ):
        """保存情感分析结果"""
    ...
    def save_knowledge(self, title, content, category, tags=None):
        """保存知识"""
        knowledge = Knowledge(
            title=title, content=content, category=category, tags=str(tags) if tags else None
        )
    ...
    def log_system_event(self, level, message, session_id=None, user_id=None):
        """记录系统日志"""
        log = SystemLog(level=level, message=message, session_id=session_id, user_id=user_id)
        self.db.add(log)
        self.db.commit()
        return log
    ...
    def delete_session(self, session_id):
        ...
        try:
            ...
            # 删除会话记录
            deleted_count = (
                self.db.query(ChatSession).filter(ChatSession.session_id == session_id).delete()
            )
    ...
    def save_evaluation(self, evaluation_data):
        ...
        evaluation = ResponseEvaluation(
            ...
            improvement_suggestions=json.dumps(
                evaluation_data.get("improvement_suggestions", []), ensure_ascii=False
            ),
    ...
    def get_evaluation_statistics(self, start_date=None, end_date=None):
        ...
        if not evaluations:
            return {
                "total_count": 0,
                "average_scores": {"empathy": 0, "naturalness": 0, "safety": 0, "overall": 0},
            }
    ...
    def update_evaluation_human_verification(self, evaluation_id, human_scores):
        """更新评估的人工验证数据"""
        evaluation = (
            self.db.query(ResponseEvaluation).filter(ResponseEvaluation.id == evaluation_id).first()
        )
        if evaluation:
