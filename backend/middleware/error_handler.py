...
class ErrorHandlerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # 处理自定义异常
            logger.error(
                f"自定义异常: {e.error_code} - {e.message}",
                extra={
            ...
            return JSONResponse(
                ...
                content=format_error(
                    error=e, error_code=e.error_code, status_code=status_code, details=e.details
                ),
            )
            ...
            # 处理FastAPI HTTP异常
            logger.warning(
                f"HTTP异常: {e.status_code} - {e.detail}",
                extra={
            ...
            return JSONResponse(
                status_code=e.status_code,
                content=format_error(
                    error=e.detail, error_code="HTTP_EXCEPTION", status_code=e.status_code
                ),
            )
