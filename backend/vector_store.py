...
class VectorStore:
    def __init__(self):
        ...
        if os.path.exists(db_path):
            try:
                # 尝试创建客户端，如果失败则删除旧数据库
                test_client = chromadb.PersistentClient(path=db_path, settings=settings)
        ...
        try:
            ...
            self.knowledge_collection = self.client.get_or_create_collection(
                name="knowledge", embedding_function=default_ef, metadata={"hnsw:space": "cosine"}
            )

            self.emotion_collection = self.client.get_or_create_collection(
                name="emotions", embedding_function=default_ef, metadata={"hnsw:space": "cosine"}
            )
        except Exception as e:
    ...
    def add_knowledge(self, text: str, category: str = "general", metadata: Dict = None):
        ...
        self.knowledge_collection.add(
            documents=[text], metadatas=[{"category": category, **(metadata or {})}], ids=[doc_id]
        )
    ...
    def add_emotion_example(self, text: str, emotion: str, intensity: float):
        ...
        self.emotion_collection.add(
            documents=[text], metadatas=[{"emotion": emotion, "intensity": intensity}], ids=[doc_id]
        )
    ...
    def get_session_history(self, session_id: str, limit: int = 10):
        """获取会话历史"""
        results = self.conversation_collection.get(where={"session_id": session_id}, limit=limit)
        return results
