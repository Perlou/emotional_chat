...
class IContextService(ABC):
    ...
    async def update_context(
        self, context: ContextInfo, new_message: str, new_emotion: EmotionResult
    ) -> ContextInfo:
        """更新上下文"""
        pass
...
class IEvaluationService(ABC):
    ...
    async def evaluate_response(
        self, user_message: str, bot_response: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """评估回复质量"""
        pass

    @abstractmethod
    async def get_evaluation_history(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        """获取评估历史"""
        pass
...
class IFeedbackService(ABC):
    ...
    async def get_feedback(
        self, user_id: Optional[str] = None, feedback_type: Optional[str] = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """获取反馈"""
        pass
