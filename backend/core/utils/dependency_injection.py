...
class Container:
    ...
    def get_all_registered(self) -> Dict[Type, str]:
        """获取所有已注册的服务"""
        return {
            service_type: dependency.lifetime for service_type, dependency in self._services.items()
        }
...
def setup_example_container():
    ...
    # 注册服务，依赖配置
    container.register_transient(ExampleService, ExampleService, {"config": ExampleConfig})
