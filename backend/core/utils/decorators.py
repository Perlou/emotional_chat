...
class RateLimiter:
    ...
    async def check_rate_limit(self, key: str, max_requests: int, time_window: timedelta) -> bool:
        """检查速率限制"""
...
def rate_limit(max_requests: int = 60, time_window: int = 60, key_func: Optional[Callable] = None):
    """速率限制装饰器"""
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            allowed = await _global_rate_limiter.check_rate_limit(
                key=limit_key, max_requests=max_requests, time_window=timedelta(seconds=time_window)
            )

            if not allowed:
                retry_after = time_window
                raise RateLimitError(
                    message=f"Rate limit exceeded for {func.__name__}", retry_after=retry_after
                )
...
def log_execution(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            # 记录开始
            log_data = {"function": func.__name__, "started_at": datetime.now().isoformat()}
            ...
            try:
                ...
                log_data.update({"execution_time": execution_time, "status": "success"})
                ...
                execution_time = time.time() - start_time
                log_data.update(
                    {"execution_time": execution_time, "status": "error", "error": str(e)}
                )
