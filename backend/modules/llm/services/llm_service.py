...
class LLMService:
    ...
    def __init__(self, config: Optional[LLMConfig] = None):
        ...
        Args:
            config: LLM配置
        """
        self.config = config or LLMConfig(api_key="", provider=LLMProvider.OPENAI)
        ...
        **kwargs,
        ...
        try:
            ...
            raise ExternalServiceError(
                message=f"LLM聊天补全失败: {str(e)}", service_name="LLM", status_code=500
            )
    ...
    async def text_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ExternalServiceError(
                message=f"LLM文本补全失败: {str(e)}", service_name="LLM", status_code=500
            )
    ...
    async def stream_chat_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ExternalServiceError(
                message=f"LLM流式聊天补全失败: {str(e)}", service_name="LLM", status_code=500
            )
...
"""
            ...
            raise ExternalServiceError(
                message=f"情绪分析失败: {str(e)}", service_name="LLM", status_code=500
            )
...
"""
            ...
            raise ExternalServiceError(
                message=f"记忆提取失败: {str(e)}", service_name="LLM", status_code=500
            )
    ...
    async def generate_response(
        ...
        **kwargs,
        ...
        try:
            # 构建系统提示
            system_prompt = f"""你是"心境"，一个专业的心理健康陪伴机器人。
...
"""
            ...
            response = await self.chat_completion(
                messages=messages, temperature=0.7, max_tokens=500, **kwargs
            )
            ...
            raise ExternalServiceError(
                message=f"回复生成失败: {str(e)}", service_name="LLM", status_code=500
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            start_time = time.time()
            response = await self.chat_completion(messages=messages, max_tokens=10, temperature=0.1)
            response_time = time.time() - start_time
