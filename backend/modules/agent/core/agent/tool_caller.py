...
class ToolCaller:
        ...
        self.registry.register(
            ...
            parameters={
                "theme": {
                    ...
                    "required": False,
                    "default": "relaxation",
                    "description": "主题（sleep/anxiety/relaxation）",
                },
                "duration": {
                    "type": "int",
                    "required": False,
        ...
        self.registry.register(
            ...
            parameters={
                "resource_type": {
                    ...
                    "required": False,
                    "default": "article",
                    "description": "资源类型（article/video/book）",
                },
        ...
        self.registry.register(
            ...
            parameters={
                "assessment_type": {
                    "type": "string",
                    "required": True,
                    "description": "评估类型（depression/anxiety/stress）",
                },
                "user_id": {"type": "string", "required": True, "description": "用户ID"},
                "urgency": {
                    "type": "string",
    ...
    async def call_with_mcp(self, mcp_message: MCPMessage) -> MCPMessage:
        """
        使用MCP协议执行工具调用（新接口）
        ...
        output_message = self.mcp_protocol.create_tool_response(
            tool_responses=tool_responses, context=mcp_message.context
        )
    ...
    async def _search_memory(
        self, query: str, user_id: str, time_range: int = 30, emotion_filter: Optional[str] = None
    ) -> Dict[str, Any]:
        """搜索记忆工具实现"""
        try:
            ...
            results = memory_hub.retrieve(query=query, user_id=user_id, context=context, top_k=5)
    ...
    async def _get_emotion_log(
        self, user_id: str, days: int = 7, emotion_type: Optional[str] = None
    ) -> Dict[str, Any]:
        """获取情绪日志工具实现"""
        try:
            from .memory_hub import get_memory_hub
    ...
    async def _set_reminder(
        self, content: str, user_id: str, schedule_time: str, repeat: bool = False
    ) -> Dict[str, Any]:
        """设置提醒工具实现"""
        try:
    ...
    async def _recommend_meditation(
        self, theme: str = "relaxation", duration: int = 10, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """推荐冥想音频工具实现"""
    ...
    async def _recommend_resource(
        self, theme: str, resource_type: str = "article", user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """推荐心理健康资源"""
        # 资源数据库（简化实现）
        resources = {
            "anxiety": {
                "article": [
                    ...
                    {
                        "title": "焦虑症的自我调节方法",
                        "url": "https://example.com/article/anxiety2",
                    },
                ],
                "video": [{"title": "焦虑管理技巧", "url": "https://example.com/video/anxiety1"}],
            },
            "depression": {
                "article": [
    ...
    async def _psychological_assessment(
        self, assessment_type: str, user_id: str, urgency: str = "medium"
    ) -> Dict[str, Any]:
        """心理健康评估工具"""
        # 这里应该返回评估问卷或触发评估流程
        assessments = {
            ...
            "anxiety": {"name": "GAD-7焦虑量表", "questions_count": 7, "estimated_time": "2分钟"},
            "stress": {"name": "压力感知量表", "questions_count": 10, "estimated_time": "5分钟"},
        }
    ...
    async def _check_calendar(
        ...
        return {
            "user_id": user_id,
            "events": [
                {"title": "重要会议", "date": "2025-10-16", "time": "14:00", "type": "work"},
                {"title": "健身", "date": "2025-10-17", "time": "18:00", "type": "personal"},
            ],
            "count": 2,
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        result = await tool_caller.call(
            "search_memory", {"query": "睡眠", "user_id": "user_123", "time_range": 7}
        )
        ...
        # 调用工具示例2：推荐冥想
        print("2. 推荐冥想：")
        result = await tool_caller.call("recommend_meditation", {"theme": "sleep", "duration": 15})
        print(json.dumps(result, ensure_ascii=False, indent=2))
