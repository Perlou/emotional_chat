...
class SchedulerService:
    ...
    def get_upcoming_reminders(self, user_id: str, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        return self.get_reminders(
            user_id=user_id, status=ReminderStatus.SCHEDULED, start_date=now, end_date=end_time
        )
        ...
        for user_id, user_reminders in self.reminders.items():
            for reminder in user_reminders:
                if (
                    ...
                    and reminder["schedule_time"] <= now
                ):
                    due_reminders.append(reminder)

        return due_reminders

    def update_reminder(self, user_id: str, reminder_id: str, updates: Dict[str, Any]) -> bool:
        """
        更新提醒
