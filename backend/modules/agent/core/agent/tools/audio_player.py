...
class AudioPlayer:
    ...
    def search_audio(
        ...
        for cat in categories:
            for audio in self.audio_library[cat]:
                ...
                results.append({**audio, "category": cat})

        return results

    def get_recommendation(
        self, user_id: str, emotion: str, duration: int = 10
    ) -> List[Dict[str, Any]]:
        """
        根据用户情绪推荐音频
        ...
        # 搜索相关音频
        results = self.search_audio(theme=theme, category="meditation", max_duration=duration + 5)
    ...
    def play_audio(self, user_id: str, audio_id: str) -> Dict[str, Any]:
        """
        播放音频
        ...
        self.play_history[user_id].append(
            {"audio_id": audio_id, "played_at": datetime.now().isoformat()}
        )

        return {"success": True, "audio": audio, "message": f"正在播放：{audio['title']}"}

    def get_play_history(self, user_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        获取播放历史
        ...
        for record in history[-limit:]:
            ...
            if audio:
                detailed_history.append({**record, "audio": audio})
