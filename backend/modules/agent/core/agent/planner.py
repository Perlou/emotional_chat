...
class Planner:
                ...
                steps=[
                    {
                        "action": "respond",
                        "tool": "llm_generate",
                        "parameters": {"user_input": user_input, "context": context},
                    }
                ],
            )
    ...
    def _identify_goal(self, user_input: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        识别用户目标
    ...
    def _build_task_graph(self, sub_goals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {"tasks": sorted_tasks, "total_count": len(sorted_tasks)}

    def _select_strategy(self, task_graph: Dict[str, Any], context: Dict[str, Any]) -> Strategy:
        """
        策略选择
    ...
    def _generate_plan(
        self, task_graph: Dict[str, Any], strategy: Strategy, context: Dict[str, Any]
    ) -> ExecutionPlan:
        """
        生成执行计划
                    ...
                    steps.append(
                        {
                            "action": "tool_call",
                            "tool": tool,
                            "parameters": self._generate_tool_parameters(tool, task, context),
                ...
                steps.append(
                    {
                        "action": "respond",
                        "tool": "llm_generate",
                        "parameters": {"task_description": task["description"], "context": context},
                        "task_id": task["task_id"],
                    }
                )
    ...
    async def plan_with_mcp(self, mcp_message: MCPMessage) -> MCPMessage:
        """
        使用MCP协议进行规划（新接口）
        ...
        context = {
            ...
            "user_id": mcp_context.user_profile.get("user_id")
            if mcp_context.user_profile
            else None,
            "perception": {
                "emotion": mcp_context.emotion_state.get("emotion")
                if mcp_context.emotion_state
                else "平静",
                "emotion_intensity": mcp_context.emotion_state.get("intensity", 5.0)
                if mcp_context.emotion_state
                else 5.0,
            },
            "memories": mcp_context.memory_summary.get("memories", [])
            if mcp_context.memory_summary
            else [],
        ...
        for step in execution_plan.steps:
            if step.get("action") == "tool_call":
                tool_call = MCPToolCall(
                    tool_name=step.get("tool", ""), parameters=step.get("parameters", {})
                )
        ...
        output_message.metadata = {
            **(output_message.metadata or {}),
            "interaction_id": mcp_message.metadata.get("interaction_id")
            if mcp_message.metadata
            else None,
    ...
    def _generate_tool_parameters(
        self, tool_name: str, task: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        生成工具调用参数
        ...
        if tool_name == "search_memory":
            ...
            return {"theme": perception.get("emotion", "relaxation"), "user_id": user_id}

        elif tool_name == "get_emotion_log":
            return {"user_id": user_id, "days": 7, "emotion_type": perception.get("emotion")}
    ...
    def _init_rules(self) -> Dict[str, Any]:
        ...
        """
        return {
            "emotion_threshold": {"high": 7.0, "medium": 5.0, "low": 3.0},
            "complexity_rules": {
                "simple": ["查询", "是什么", "定义"],
                "medium": ["怎么办", "建议"],
                "complex": ["计划", "改变", "系统"],
...
if __name__ == "__main__":
    ...
    context = {
        "user_id": "user_123",
        "perception": {"emotion": "焦虑", "emotion_intensity": 7.5, "intent": "problem_solving"},
        "memories": [],
    }
