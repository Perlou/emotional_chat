...
class MemoryHub:
    ...
    def retrieve(
        self, query: str, user_id: str, context: Optional[Dict[str, Any]] = None, top_k: int = 5
    ) -> List[Dict[str, Any]]:
        """
        检索：基于查询和上下文检索相关记忆
        ...
        try:
            semantic_results = self.memory_manager.search_memories(
                user_id=user_id, query=query, top_k=top_k, min_importance=0.3
            )
        ...
        if context.get("time_range"):
            try:
                temporal_results = self._search_recent(
                    user_id=user_id, days=context.get("time_range", 7), limit=3
                )
        ...
        if context.get("emotion"):
            try:
                emotion_results = self._search_by_emotion(
                    user_id=user_id, emotion=context["emotion"], limit=2
                )
    ...
    def clear_working_memory(self):
        """清空工作记忆"""
        self.working_memory = {"conversation": [], "active_tasks": [], "temp_variables": {}}
    ...
    def get_action_log(self, user_id: str, days: int = 7, limit: int = 10) -> List[Dict[str, Any]]:
        """
        获取用户行为日志
        ...
        try:
            ...
            messages = (
                db.query(Message)
                .join(Conversation)
                .filter(Conversation.user_id == user_id, Message.created_at >= since_date)
    ...
    def _save_conversation_memory(self, memory: Dict[str, Any]):
        ...
        try:
            ...
            conversation = (
                ...
                .filter(Conversation.user_id == memory["user_id"])
    ...
    def _search_recent(self, user_id: str, days: int = 7, limit: int = 5) -> List[Dict[str, Any]]:
        """搜索近期记忆"""
        try:
            db = next(get_db())
            since_date = datetime.now() - timedelta(days=days)
    ...
    def _search_by_emotion(
        self, user_id: str, emotion: str, limit: int = 3
    ) -> List[Dict[str, Any]]:
        """根据情绪搜索记忆"""
        try:
            db = next(get_db())
    ...
    def _merge_and_rank(self, results: List[Dict[str, Any]], top_k: int) -> List[Dict[str, Any]]:
        """合并去重并排序记忆"""
        # 简单去重（基于内容）
        seen_contents = set()
        unique_results = []
        ...
        # 按重要性和时间排序
        unique_results.sort(
            key=lambda x: (x.get("importance", 0.5), x.get("timestamp", datetime.min)), reverse=True
        )
    ...
    def _extract_personality_traits(self, user_id: str) -> List[str]:
        ...
        try:
            ...
            emotion_map = {"焦虑": "敏感型", "开心": "乐观型", "难过": "情绪化", "平静": "稳重型"}
