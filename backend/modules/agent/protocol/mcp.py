...
class MCPProtocol:
    ...
    def create_tool_request(
        tool_calls: List[MCPToolCall], context: Optional[MCPContext] = None
    ) -> MCPMessage:
        """
        创建工具请求MCP消息
    ...
    def create_tool_response(
        tool_responses: List[MCPToolResponse], context: Optional[MCPContext] = None
    ) -> MCPMessage:
        """
        创建工具响应MCP消息
    ...
    def create_reflector_evaluation(
        content: str, evaluation_result: Dict[str, Any], context: Optional[MCPContext] = None
    ) -> MCPMessage:
        """
        创建Reflector评估MCP消息
    ...
    @staticmethod
    def merge_context(base_context: MCPContext, additional_context: MCPContext) -> MCPContext:
        """
        合并上下文
...
if __name__ == "__main__":
    ...
    user_msg = protocol.create_user_input(
        content="我最近心情很不好，感觉很焦虑", emotion_state={"emotion": "焦虑", "intensity": 7.5}
    )
    print("用户输入消息（静态方法）：")
    ...
    tool_call = MCPToolCall(
        tool_name="search_memory", parameters={"query": "焦虑", "user_id": "user_123"}
    )
    ...
    tool_response_msg = protocol.create_tool_response(tool_responses=[tool_response])
    print("工具响应消息：")
