...
class KnowledgeBaseManager:
    ...
    def __init__(
        ...
        self.strategy_selector = ChunkingStrategySelector(
            default_strategy=chunking_strategy, chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )

        logger.info(
            f"知识库管理器初始化完成，持久化目录: {persist_directory}, 分块策略: {chunking_strategy}"
        )
    ...
    def load_directory_documents(
        self, directory_path: str, glob_pattern: str = "**/*.pdf"
    ) -> List[Document]:
        """
        批量加载目录下的文档
        ...
        try:
            ...
            loader = DirectoryLoader(
                directory_path, glob=glob_pattern, loader_cls=PyPDFLoader, show_progress=True
            )
            documents = loader.load()
    ...
    def split_documents(
        self, documents: List[Document], strategy: Optional[str] = None
    ) -> List[Document]:
        """
        分割文档为小块
        ...
        try:
            ...
            use_strategy_selector = (
                ...
                or self.chunking_strategy == "auto"
                or self.chunking_strategy not in ["recursive", "character", "sentence"]
            )

            if use_strategy_selector:
                # 使用策略选择器
                actual_strategy = strategy or self.chunking_strategy
                chunks = self.strategy_selector.split_documents(documents, strategy=actual_strategy)
            else:
                # 使用传统分块器（向后兼容）
    ...
    def load_from_knowledge_base_structure(self, base_path: str = "./knowledge_base") -> None:
        ...
        try:
                ...
                if os.path.exists(folder_path):
                    ...
                    try:
                        docs = self.kb_manager.load_directory_documents(
                            folder_path, glob_pattern="**/*"
                        )

                        # 为每个文档添加分类元数据
                        for doc in docs:
                            doc.metadata.update(
                                {"category": category, "folder": folder, "source": "知识库文件"}
                            )
            ...
            if all_documents:
                ...
                logger.info(
                    f"成功从知识库结构加载知识，共 {len(all_documents)} 个文档，{len(chunks)} 个文档块"
                )
            else:
