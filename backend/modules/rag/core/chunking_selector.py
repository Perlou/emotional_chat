...
class ChunkingStrategySelector:
    ...
    def __init__(
        self, default_strategy: str = "auto", chunk_size: int = 500, chunk_overlap: int = 50
    ):
        """
        初始化策略选择器
    ...
    def _initialize_splitters(self):
        ...
        self.splitters = {
            ...
            "dialogue": DialogueSplitter(
                max_turns=10, max_chars=self.chunk_size * 2, overlap_turns=2
            ),
    ...
    def detect_document_type(self, text: str) -> Dict[str, Any]:
        ...
        markdown_score = sum(
            1 for pattern in markdown_patterns if re.search(pattern, text, re.MULTILINE)
        )
        ...
        dialogue_pattern = re.compile(
            r"^(?:User|用户|Assistant|助手|AI|机器人|问|答)[:：]\s*", re.MULTILINE
        )
