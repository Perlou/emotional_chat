...
class MarkdownStructureSplitter:
    ...
    def __init__(self, chunk_size: int = 900, min_chunk: int = 250, overlap_ratio: float = 0.1):
        """
        初始化Markdown结构分块器
    ...
    def split_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        for ch in chunks:
            ...
            if (
                len(ch["text"]) < self.min_chunk
                and merged[-1]["meta"]["breadcrumbs"] == ch["meta"]["breadcrumbs"]
            ):
                merged[-1]["text"] += "\n\n" + ch["text"]
            else:
                merged.append(ch)
...
class DialogueSplitter:
    ...
    def __init__(self, max_turns: int = 10, max_chars: int = 900, overlap_turns: int = 2):
        """
        初始化对话分块器
...
class SmallBigChunking:
    ...
    def __init__(
        ...
        self.small_splitter = SentenceTextSplitter(
            chunk_size=small_chunk_size, chunk_overlap=small_overlap
        )
