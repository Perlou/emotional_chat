...
class RAGService:
    ...
    def __init__(self, kb_manager: Optional[KnowledgeBaseManager] = None):
        ...
        self.prompt_template = PromptTemplate(
            template="""你是"心境"，一个专业的心理健康陪伴机器人。你正在使用专业的心理学知识库来回答用户的问题。
...
回答：""",
    ...
    def ask_with_context(
        ...
        try:
            ...
            # 构建增强的上下文
            knowledge_context = "\n\n".join(
                [f"【知识{i + 1}】{doc.page_content}" for i, doc in enumerate(knowledge_docs)]
            )
            ...
            if conversation_history:
                ...
                for msg in recent_history:
                    role = "用户" if msg.get("role") == "user" else "心境"
            ...
            # 构建完整的prompt
            enhanced_prompt = f"""你是"心境"，一个专业的心理健康陪伴机器人。
...
class RAGIntegrationService:
    ...
    def should_use_rag(self, message: str, emotion: Optional[str] = None) -> bool:
        ...
        # 需要专业建议的情绪
        professional_emotions = ["焦虑", "抑郁", "压力大", "紧张", "恐惧", "悲伤", "愤怒"]
    ...
    def enhance_response(
        ...
        try:
            ...
            if not self.should_use_rag(message, emotion):
                return {"use_rag": False, "reason": "当前对话不需要专业知识库支持"}
            ...
            return {"use_rag": False, "error": str(e)}
