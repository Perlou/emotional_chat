...
class MemoryService:
        ...
        try:
            with DatabaseManager() as db:
                for memory in memories:
                    # 检查是否已存在
                    existing = (
                        db.db.query(MemoryItem)
                        .filter(MemoryItem.memory_id == memory.get("id"))
                        .first()
                    )
    ...
    async def retrieve_memories(
        self, user_id: str, query: str, n_results: int = 3, days_limit: int = 7
    ) -> List[Dict[str, Any]]:
        """
        检索相关记忆
        ...
        memories = self.memory_manager.retrieve_memories(
            user_id=user_id, query=query, n_results=n_results, days_limit=days_limit
        )
        ...
        try:
            with DatabaseManager() as db:
                for memory in memories:
                    memory_id = memory.get("id")
                    memory_item = (
                        db.db.query(MemoryItem).filter(MemoryItem.memory_id == memory_id).first()
                    )
        ...
        if success:
            try:
                with DatabaseManager() as db:
                    memory_item = (
                        db.db.query(MemoryItem)
                        .filter(MemoryItem.memory_id == memory_id, MemoryItem.user_id == user_id)
                        .first()
                    )
    ...
    async def get_memory_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            with DatabaseManager() as db:
                ...
                # 总记忆数
                total_count = (
                    db.db.query(func.count(MemoryItem.id))
                ...
                # 按类型统计
                type_stats = (
                    db.db.query(MemoryItem.memory_type, func.count(MemoryItem.id).label("count"))
                    .filter(MemoryItem.user_id == user_id, MemoryItem.is_active == True)
                ...
                # 平均重要性
                avg_importance = (
                    db.db.query(func.avg(MemoryItem.importance))
                    .filter(MemoryItem.user_id == user_id, MemoryItem.is_active == True)
                    .scalar()
                )
                ...
                return {
                    ...
                    "by_type": [{"type": t.memory_type, "count": t.count} for t in type_stats],
                }
        except Exception as e:
            print(f"获取记忆统计失败: {e}")
