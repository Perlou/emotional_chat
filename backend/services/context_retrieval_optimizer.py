...
class FileSystemRetriever:
    ...
    def grep_search(
        self, pattern: str, file_pattern: str = "*.json", directory: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        使用grep搜索文件内容
    ...
    def search_context_by_keywords(
        self, keywords: List[str], context_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        根据关键词搜索上下文
            ...
            results.append(
                {
                    ...
                    "matched_keywords": [
                        kw for kw in keywords if kw.lower() in result["content"].lower()
                    ],
                }
            )
...
class ContextRetrievalOptimizer:
    ...
    def __init__(
        self, file_retriever: Optional[FileSystemRetriever] = None, use_vector_search: bool = False
    ):
        """
        初始化检索优化器
    ...
    def retrieve_relevant_context(
        self, query: str, context_type: Optional[str] = None, max_results: int = 5
    ) -> List[Dict[str, Any]]:
        """
        检索相关上下文
        ...
        # 使用文件系统搜索
        results = self.file_retriever.search_context_by_keywords(keywords, context_type)
