...
class ContextCompactionStrategy:
    ...
    def compact_tool_call(self, tool_call: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if tool_call.get("tool_name") in ["write_file", "read_file", "create_file"]:
            file_path = tool_call.get("arguments", {}).get("path") or tool_call.get(
                "arguments", {}
            ).get("file_path")
...
class ContextSummarizer:
    ...
    def summarize_conversation_turns(
        self, turns: List[Dict[str, Any]], schema: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        摘要对话轮次 - 使用结构化模式确保一致性
    ...
    def _llm_enhance_summary(
        self, summary: Dict[str, Any], turns: List[Dict[str, Any]], schema: Dict[str, Any]
    ) -> Dict[str, Any]:
        """使用LLM增强摘要（可选）"""
        # TODO: 实现LLM增强摘要
        return summary
...
class ContextRotSolver:
    ...
    def reduce_context(
        self, context: Dict[str, Any], preserve_recent_turns: int = 5
    ) -> Dict[str, Any]:
        """
        缩减上下文 - 先压缩，再摘要
    ...
    def _apply_compaction(self, context: Dict[str, Any], preserve_recent: int) -> Dict[str, Any]:
        """
        应用压缩策略
        ...
        if "tool_calls" in compacted:
            ...
            # 保留最近的工具调用完整，压缩旧的
            recent_calls = (
                tool_calls[-preserve_recent:] if len(tool_calls) > preserve_recent else tool_calls
            )
    ...
    def _apply_summarization(self, context: Dict[str, Any], preserve_recent: int) -> Dict[str, Any]:
        """
        应用摘要策略
        ...
        if "chat_history" in summarized:
            ...
            if len(history) > preserve_recent * 2:
                ...
                summarized["chat_history"] = [
                    {
                        "role": "system",
                        "content": f"早期对话摘要：{json.dumps(summary, ensure_ascii=False)}",
                    }
                ] + to_preserve
    ...
    def offload_to_file(
        self, context: Dict[str, Any], identifier: str
